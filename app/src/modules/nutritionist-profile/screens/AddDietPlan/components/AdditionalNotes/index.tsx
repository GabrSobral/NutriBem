...
export function AdditionalNotes() {
  ...
  async function addNote() {
    dietPlanDispatch({
      ...
      payload: {
        id: (dietPlanState.additionalNotes.length + 1).toString(),
        note,
        order: dietPlanState.additionalNotes.length,
      },
    })
  ...
  return (
    <View style={{ gap: 10 }}>
      ...
      <NestableDraggableFlatList
        data={dietPlanState.additionalNotes}
        keyExtractor={(item) => item.id}
        onDragEnd={({ data }) =>
          dietPlanDispatch({ type: "SET_ADDITIONAL_NOTES", payload: data })
        }
        style={[
